...
import cascadenet.network.theano_extensions.fft
...
if scikits_cuda_available:
    ...
    @op_lifter([cascadenet.network.theano_extensions.fft.FFTOp])
    @register_opt2([cascadenet.network.theano_extensions.fft.FFTOp], 'fast_compile')
    def local_gpua_cufft_op(op, ctx_name, inputs, outputs):
        return cufft_op

    # @op_lifter([theano.tensor.fft.IFFTOp])
    # @register_opt2([theano.tensor.fft.IFFTOp], 'fast_compile')
    @register_opt('fast_compile')
    @op_lifter([cascadenet.network.theano_extensions.fft.IFFTOp])
    @register_opt2([cascadenet.network.theano_extensions.fft.IFFTOp], 'fast_compile')
    def local_gpua_cuifft_op(op, ctx_name, inputs, outputs):
        return cuifft_op
